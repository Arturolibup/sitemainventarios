...
class VehiculoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Car::query()
            ->with(['marca:id,nombre', 'tipo:id,nombre', 'subarea:id,name,area_id',])
            ->whereNull('deleted_at');

        if ($search !== '') {
            $query->where(function($q) use ($search) {
                $q->where('placa', 'like', "%{$search}%")
                  ->orWhere('numero_eco', 'like', "%{$search}%")
                  ->orWhere('modelo', 'like', "%{$search}%")
                  ->orWhere('cilindro', 'like', "%{$search}%")
                  ->orWhere('numero_serie', 'like', "%{$search}%")
                  ->orWhereHas('marca', function ($qq) use ($search) {
                       $qq->where('nombre', 'like', "%{$search}%");
                  })
                  ->orWhereHas('tipo', function ($qq) use ($search) {
                       $qq->where('nombre', 'like', "%{$search}%");
                  })
                  ->orWhereHas('subarea', function ($qq) use ($search) {
                       $qq->where('name', 'like', "%{$search}%");
                  });
            });
        }

        $vehiculosPag = $query->orderBy('id', 'desc')->paginate($perPage, ['*'], 'page', $page);

        // Formatear respuesta simple para el frontend
        $vehiculos = $vehiculosPag->items();

        // Mapear campos principales (mantener estructura útil)
        $formatted = array_map(function($v) {
            return [
                ...
                'marca_id' => $v->marca_id,
                'marca' => $v->marca ? ['id' => $v->marca->id, 'nombre' => $v->marca->nombre] : null,
                'tipo_id' => $v->tipo_id,
                'tipo' => $v->tipo ? ['id' => $v->tipo->id, 'nombre' => $v->tipo->nombre] : null,
                'modelo' => $v->modelo,
                'cilindro' => $v->cilindro,
                'subarea_asigna' => $v->subarea_asigna,
                'subarea' => $v->subarea ? ['id' => $v->subarea->id, 'name' => $v->subarea->name] : null,
                'created_at' => $v->created_at ? $v->created_at->format('Y-m-d H:i:s') : null,
                'updated_at' => $v->updated_at ? $v->updated_at->format('Y-m-d H:i:s') : null,
            ];
        }, $vehiculos);

        return response()->json([
            'meta' => [
                'total' => $vehiculosPag->total(),
                'per_page' => $vehiculosPag->perPage(),
                'current_page' => $vehiculosPag->currentPage(),
                'last_page' => $vehiculosPag->lastPage(),
            ],
            'vehiculos' => $formatted
        ], 200);
    }
