...
ngAfterViewInit(): void {
  // Solución definitiva al overlay gris bloqueado en Metronic + Angular
  setTimeout(() => {
    // 1. Quitar splash oficial
    try {
      if (typeof KTApp !== 'undefined') {
        KTApp.hidePageLoading();
      }
      const splash = document.getElementById('kt_splash_screen');
      if (splash) splash.style.display = 'none';
    } catch (e) {}

    // 2. QUITAR LA CLASE QUE CAUSA EL OVERLAY GRIS
...
/*
  //yo agregue esto por que se pone oscuro
  ngAfterViewInit(): void {
  // ✅ Espera medio segundo y oculta el splash screen o loader global
  setTimeout(() => {
    try {
      KTApp.hidePageLoading();
      const splash = document.getElementById('kt_splash_screen');
      ...
      console.warn('Loader no encontrado:', e);
    }
  }, 500);
}
*/