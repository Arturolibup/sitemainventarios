...
@Component({
  ...
  styleUrls: ['./op-create.component.scss']
})

export class OpCreateComponent implements OnInit, OnDestroy {
...
//para creacion de productos
  ...
  ngOnInit(): void {
    ...
    if (!hasPermission) {
      Swal.fire('Error', 'No tienes permisos para acceder A ESTE MODULO.', 'error');
...
resetSubareaSearch(): void {
  ...
  selectProduct(product: any, index: number, event?: Event): void {
    if (event) {
      event.stopPropagation(); // Para el botón
    }
...
submit(): void {
  ...
  if (this.orderForm.invalid) {
    ...
    Swal.fire({
      title: 'Error',
      html: errors.length
        ? `Corrige los siguientes errores:<br>${errors.join('<br>')}`
  ...
  formValue.requester_subarea_id = formValue.requester_subarea_id
    ? Number(formValue.requester_subarea_id)
    : null;

  // 3) Determinar status según el modo
  let computedStatus: string | null = formValue.status || null;
  // STATUS EN BD: 'pending_sf_validation','validate_sf','pending_warehouse','partially_received','completed'
  ...
  if (this.mode === 'create_sf') {
    ...
    const allReceived = items.length > 0 &&
      items.every(p => p.received_quantity >= p.quantity && p.quantity > 0);
    const anyReceived = items.some(p => p.received_quantity > 0);
    computedStatus = allReceived
      ? 'completed'
      : anyReceived
        ? 'partially_received'
        : 'pending_warehouse';
  ...
  const cleanedFormValue = {
    ...
    products: formValue.products.map((product: any) => {
      ...
      if (!rawProductId) {
        console.error('Producto sin product_id:', product);
      }

      return {
        id: product.id || null,
        product_id: rawProductId ? Number(rawProductId) : null,
        ...
        vehicle_id: product.vehicle_id ? Number(product.vehicle_id) : null,
  ...
  console.log(
    'SUBMIT financials -> concept_total(form):',
    this.orderForm.get('concept_total')?.value,
    'iva(form):',
    this.orderForm.get('iva')?.value,
    'isr(form):',
    this.orderForm.get('isr_retention')?.value,
    'total(form):',
    this.orderForm.get('total')?.value,
    'financialDisplays:',
    this.financialDisplays,
    'ivaEditing:',
    this.ivaEditing,
  ...
  cleanedFormValue.products.forEach((product: any) => {
    ...
    delete product.amount; // lo recalcula backend si quiere
  });

  let action: Observable<any>;
  let successMessage = '';

  // 5) Selección de acción según modo y permisos
  if (this.mode === 'create_sf' && this.authService.hasPermission('orders.create_sf')) {
    ...
    successMessage =
      cleanedFormValue.status === 'completed'
  ...
  action.pipe(takeUntil(this.destroy$)).subscribe({
    next: (response) => {
      ...
      if (this.mode === 'create_sf') {
        ...
        notify(
          `Orden ${ordNum} enviada a Almacén para recepción (estatus: ${cleanedFormValue.status}).`
        );
      } else if (this.mode === 'receive') {
        notify(
          `Recepción ${
            cleanedFormValue.status === 'completed' ? 'COMPLETA' : 'PARCIAL'
          } registrada para orden ${ordNum}.`
        );
      }

      // Generar PDFs
      if (this.mode === 'create_sf' || this.mode === 'validate_sf') {
        this.generateAndSaveSuficienciaPdf(orderId as number);
      } else if (this.mode === 'add_order_number' || this.mode === 'validate') {
        this.generateAndSaveOrderPdf(orderId as number);
      }

      Swal.fire({
        title: 'Éxito',
        text: `${successMessage} ¿Deseas ${
          this.mode === 'validate_sf' || this.mode === 'create_sf'
            ? 'crear otra'
            : 'volver a la lista'
        }?`,
        icon: 'success',
        ...
        confirmButtonText:
          this.mode === 'validate_sf' || this.mode === 'create_sf' ? 'SÍ' : 'Volver',
        cancelButtonText:
          this.mode === 'validate_sf' || this.mode === 'create_sf' ? 'NO' : 'Quedarme',
      }).then((result) => {
        this.setGlobalLoading(false);
...
generateAndSaveOrderPdf(orderId: number): void {
  ...
  if (this.user && this.user.id && this.orderForm.get('id')?.value) {
    this.orderService.createNotification({
      user_id: this.user.id,
      order_request_id: this.orderForm.get('id')!.value,
      message,
      }).subscribe({
  ...
  uploadInvoice() {
    ...
    input.onchange = (event: any) => {
      ...
      if (file) {
        ...
        formData.append('order_request_id', this.orderForm.get('id')!.value);
...
// ✅ Seleccionar vehículo
  selectVehiculo(vehiculo: any, event?: Event): void {
    ...
    productForm.patchValue({
      ...
      numero_eco: vehiculo.numero_eco || '',
...
/*nuevas agregadas 12/10/25*
 * calculateProductAmountsForIndex(index)
 * --------------------------------------
 * Calcula amount para un solo renglón y luego actualiza totales generales.
 * Evita emitir eventos innecesarios.
 */
...
/**
 * Reemplaza la función calculateProductAmounts()
 * ------------------------------------------------
 * Recalcula amounts por producto y concept_total.
 * - Parchea solo cuando el valor cambia.
 * - Si isAutoCalculation === true recalcula IVA automáticamente.
 * - Si isAutoCalculation === false respeta los valores de IVA/ISR y recalcula total.
 */
...
// trackBy para units (mejora rendimiento y evita re-renders que rompen selección)
trackByUnit = (_: number, item: any) => item?.id ?? _;

/**
 * Usado por [compareWith] para comparar primitive numbers/strings de forma segura.
 */
