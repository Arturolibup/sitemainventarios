...
export class ListRolesComponent {
 ...
 createRol(){
   const modalRef = this.modalService.open(CreateRolesComponent, {centered:true,size: 'md'});
    modalRef.componentInstance.RoleC.subscribe((role:any) =>{
      this.ROLES.unshift(role); //unshift para que aparezca primero
 ...
 editRole(ROL:any){
  const modalRef = this.modalService.open(EditRolesComponent, {centered:true,size: 'md'});
    modalRef.componentInstance.ROLE_SELECTED = ROL;  // se envia a edit roles mediante Input
     
    modalRef.componentInstance.RoleE.subscribe((role:any) =>{
      let INDEX = this.ROLES.findIndex((rol:any)=> rol.id == ROL.id); //en que posicion se encuentra el rol para editar
        if (INDEX!= -1){
 ...
 deleteRole(ROL:any){
  const modalRef = this.modalService.open(DeleteRolesComponent, {centered:true,size: 'md'});
    ...
    modalRef.componentInstance.RoleD.subscribe((role:any) =>{
      let INDEX = this.ROLES.findIndex((rol:any)=> rol.id == ROL.id); //encontrar la posicion del rol a elimar
        if (INDEX!= -1){
          this.ROLES.splice(INDEX,1);  //eliminacion de un permiso ahora en un rol
        }
      })
 }
...
refreshRoles(): void {
  ...
  this.listRoles(); // Vuelve a cargar toda la lista de roles
  }
}
